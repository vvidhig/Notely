...
export default function HighlightToolbar({ selection, onHighlight, onCreateTask, onDismiss }: Props) {
  ...
  return (
    <div
      ref={ref}
      className="fixed z-50 flex items-center gap-1 bg-[#1E3550] rounded-xl px-2 py-1.5 shadow-[0_4px_24px_rgba(0,0,0,0.25)]"
      style={{ top: Math.max(top, 8), left: Math.max(left, 8) }}
    >
      <button
        onMouseDown={(e) => { e.preventDefault(); onHighlight(selection); }}
        className="flex items-center gap-1.5 text-yellow-300 hover:text-yellow-100 text-xs font-bold px-2 py-1 rounded-lg hover:bg-white/10 transition-all"
      >
        <Highlighter size={13} />
        Highlight
      </button>
      ...
      <button
        ...
        className="flex items-center gap-1.5 text-emerald-300 hover:text-emerald-100 text-xs font-bold px-2 py-1 rounded-lg hover:bg-white/10 transition-all"
      >
        <Plus size={13} />
        Task
      </button>
